...
public class MyMesh : MonoBehaviour
{
    ...
    //public Mesh myMesh;
    [HideInInspector]public Material TerrainMat;
    ...
    void MakeAlbedoAtlas()
    {
        ...
        albedoAtlas = new Texture2DArray(wid, hei, sqrCount * sqrCount, normalTerrainData.terrainLayers[0].diffuseTexture.format, true, false);
        ...
        for (int i = 0; i < sqrCount; i++)
        {
            for (int j = 0; j < sqrCount; j++)
            {
                ...
                for (int k = 0; k < normalTerrainData.terrainLayers[index].diffuseTexture.mipmapCount; k++)
                {
                    Graphics.CopyTexture(normalTerrainData.terrainLayers[index].diffuseTexture, 0, k, albedoAtlas, index, k);
